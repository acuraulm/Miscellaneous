    ...
    )
{
    ...
    if (!Process->PagingData->Data.KernelSpace)
    {
        ...
        if (bProcessIniialThread)
        {
			...
			Process->entryPoint = Process->HeaderInfo->Preferred.AddressOfEntryPoint;
    ...
    )
{
    ...
    LOG("Thread %x with time %x exits.", pThread->Id, pThread->creationTime);

    LockAcquire(&m_threadSystemData.AllThreadsLock, &oldState);
	RemoveEntryList(&pThread->AllList);
	RemoveEntryList(&pThread->OrderedList);
    ...
    )
{
	...
	status = ForEachElementExecute(&m_threadSystemData.OrderedThreadsList,
		Function,
		Context,
		FALSE
	);
    ...
    )
{
    ...
    __try
    {
		...
		InsertTailList(&m_threadSystemData.OrderedThreadsList, &pThread->OrderedList);
        LockRelease(&m_threadSystemData.AllThreadsLock, oldIntrState);

		LOG("Thread %x with time %x entered.", pThread->Id, pThread->creationTime);
    }
    __finally
    {
        if (!SUCCEEDED(status))
    ...
    )
{
    ...
    *ResultingStack = (PVOID) PtrDiff(InitialStack, 0x108);
	...
	Process->stackAddress = ResultingStack;
	Process->stackSize = sizeof(ResultingStack);
